...
function xMenuItem:AddGridPanel(topText, bottomText, leftText, rightText)
    ...
    item.gridPanel = { type = 'default', xAxis = 0.5, yAxis = 0.5, topText = topText, bottomText = bottomText, leftText = leftText, rightText = rightText }
...
function xMenuItem:SetGridPanelAxis(x, y)
    local item, parentMenu = getMenuItemFromId(self.menu, self.id)
    if not item or not item.gridPanel then return end
    if x ~= nil and (x < 0.0 or x > 1.0) then return end
    if y ~= nil and (y < 0.0 or y > 1.0) then return end
...
function xMenu:AddList(label, content, description, index)
    ...
    self.items[#self.items + 1] = { type = 'list', id = id, label = label, description = description, content = content, listIndex = index or 1 }
    self.eventBuses[id] = itemInstance
...
function xMenu:AddCheckbox(label, checked, description)
    ...
    self.items[#self.items + 1] = { type = 'checkbox', id = id, label = label, description = description, checked = checked }
    self.eventBuses[id] = itemInstance
...
---@param submenu xMenu
function xMenu:AddSubmenu(submenu)
...
IMPOSTEUR.On('nui:ready', function()
    ...
    local frame = IMPOSTEUR.AddFrame(frameName, ('https://cfx-nui-%s/client/libs/xmenu/dist/index.html'):format(CURRENT_RESOURCE), false)
    ...
    frame:OnMessage(function(data, cb)
        ...
        if data.action == 'closed' then
            ...
            elseif data.action == 'gridUpdated' then
                local x, y = data.x + .0, data.y + .0

                if data.x ~= nil then item.gridPanel.xAxis = x end
                if data.y ~= nil then item.gridPanel.yAxis = y end
