...
function IMPOSTEUR.GetCurrency(currencyName)
    assert(type(currencyName) == "string", ("IMPOSTEUR.GetCurrency expects 'currencyName' as a 'string'"):format(currencyName))
...
function IMPOSTEUR.SecondsToLocalizedString(seconds, minUnits, noZeros)
    ...
    if hours > 0 or (not noZeros and sexyDateStrLen > 0) then
        sexyDateStr = ('%s%s%u heure%s'):format(sexyDateStr, sexyDateStrLen > 0 and ' ' or '', hours, hours > 1 and 's' or '')
        unitsAdded += 1
    end

    sexyDateStrLen = sexyDateStr:len()
    if (minutes > 0 or (not noZeros and sexyDateStrLen > 0)) and unitsAdded < minUnits then
        sexyDateStr = ('%s%s%u minute%s'):format(sexyDateStr, sexyDateStrLen > 0 and ' ' or '', minutes, minutes > 1 and 's' or '')
        unitsAdded += 1
    end

    sexyDateStrLen = sexyDateStr:len()
    if unitsAdded < minUnits then
        sexyDateStr = ('%s%s%u seconde%s'):format(sexyDateStr, sexyDateStrLen > 0 and ' ' or '', seconds, seconds > 1 and 's' or '')
        unitsAdded += 1
    end
