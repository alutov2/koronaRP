...
function deletePlate()
    local total = 0 
    MySQL.Async.fetchAll("SELECT info, id FROM lc_trunk", {}, function(trunk)
        if trunk[1] then
            for k, v in pairs(trunk) do 
                ...
                if OwnerVeh[data.plate] ~= nil then
                    ...
                    total = total + 1
                    MySQL.Async.fetchAll("DELETE FROM lc_trunk WHERE id = @id", {
                        ['@id'] = v.id
                    }, function(result)
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    MySQL.Async.fetchAll("SELECT * FROM lc_trunk", {}, function(result)
        for k, v in pairs(result) do
...
function saveTrunk()
    ...
    for k, v in pairs(VehCoffre) do
        MySQL.Sync.execute("UPDATE lc_trunk set info = @info, data = @data WHERE id = @id", {
            ["@id"] = v.id,
...
--     MySQL.Async.execute("UPDATE lc_clothes SET identifier = @identifier, trunk = @trunk WHERE id = @id", {
...
CreateVehToBdd = function(plate, class)
    ...
    MySQL.Async.execute("INSERT INTO lc_trunk (info, id) VALUES (@info, @id)", {
